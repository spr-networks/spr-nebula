...
# lookups (registry manifest inspect, go.dev, api.github.com). Review with `git diff`.
...
# nebula: latest release tag -> full commit hash (annotated tags are dereferenced)
read -r NEBULA_VERSION NEBULA_COMMIT < <(
  python3 - "$NEBULA_REPO" <<'EOF'
import json, sys, urllib.request
repo = sys.argv[1]
def gh(path):
    req = urllib.request.Request(
        f"https://api.github.com/repos/{repo}{path}",
        headers={"Accept": "application/vnd.github+json", "User-Agent": "update-pins"})
    with urllib.request.urlopen(req) as r:
        return json.load(r)
tag = gh("/releases/latest")["tag_name"]
obj = gh(f"/git/refs/tags/{tag}")["object"]
while obj["type"] == "tag":
    obj = gh(f"/git/tags/{obj['sha']}")["object"]
assert obj["type"] == "commit", obj
print(tag.lstrip("v"), obj["sha"])
EOF
)
echo "nebula ${NEBULA_VERSION} @ ${NEBULA_COMMIT}" >&2
