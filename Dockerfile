...
ARG NEBULA_COMMIT=f573e8a26695278f9d71587390fbfe0d0933aa21
ARG TARGETARCH
...
# nebula + nebula-cert from source, pinned to the release commit (full hash).
RUN set -eux; \
    git init -q /nebula-src; \
    cd /nebula-src; \
    git remote add origin https://github.com/slackhq/nebula.git; \
    git fetch -q --depth 1 origin "${NEBULA_COMMIT}"; \
    git checkout -q "${NEBULA_COMMIT}"
RUN --mount=type=tmpfs,target=/root/go/ set -eux; \
    cd /nebula-src; \
    go build -trimpath -ldflags "-s -w -X main.Build=${NEBULA_VERSION}" -o /nebula ./cmd/nebula; \
    go build -trimpath -ldflags "-s -w -X main.Build=${NEBULA_VERSION}" -o /nebula-cert ./cmd/nebula-cert
WORKDIR /code
COPY code/ /code/
RUN --mount=type=tmpfs,target=/root/go/ go build -trimpath -ldflags "-s -w" -o /nebula_plugin /code/
...
RUN set -eux; \
    printf 'Types: deb\nURIs: https://snapshot.ubuntu.com/ubuntu/%s\nSuites: noble noble-updates noble-security\nComponents: main restricted universe multiverse\nSigned-By: /usr/share/keyrings/ubuntu-archive-keyring.gpg\n' "${UBUNTU_SNAPSHOT}" > /etc/apt/sources.list.d/ubuntu.sources; \
    printf 'APT::Install-Recommends "false";\nAcquire::Check-Valid-Until "false";\n' > /etc/apt/apt.conf.d/99reproducible
RUN apt-get update && apt-get install -y --no-install-recommends iputils-ping iproute2 && rm -rf /var/lib/apt/lists/* /var/log/* /var/cache/ldconfig/aux-cache
COPY scripts /scripts/
COPY --from=builder /nebula /nebula
COPY --from=builder /nebula-cert /nebula-cert
COPY --from=builder /nebula_plugin /
